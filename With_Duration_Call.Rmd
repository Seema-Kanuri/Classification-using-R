...
```{r}
final_var_fs <- c("duration","euribor3m","cons.price.idx","month_mar","cons.conf.idx","pdaysBucket_1 week","contact_telephone","month_may","pdaysBucket_2 weeks","poutcome_failure","month_oct",
...
#So this model consider duration into account


```{r}
final_step1 <- subset(final_step, select = -c(`nr.employed`))
```
...
library(DMwR)
data_train_smote <- SMOTE(y_yes~.,data_train,perc.over = 500, k = 5, perc.under = 200)
...
data_train_smote$y_yes <- make.names(data_train_smote$y_yes)
